...
预期:`Executed 38 tests, with 0 failures`(22 个 Scheduler + 15 个 Config + 1 个冒烟)。
...
预期:`Executed 39 tests, with 0 failures`;release 构建通过(OverlayController 随可执行 target 编译,暂无调用方,Task 6 接线)。
...
预期:测试数不变(39),编译绿。此时应用逻辑完整,只差打包成 .app。
...
预期:`Executed 38 tests, with 0 failures`(冒烟测试已删)。
...
### Task 9:休息结束后自动锁屏(lock_after_rest)

**Files:**
- Modify: `Sources/RestEyesCore/Config.swift`
- Modify: `Sources/RestEyesCore/BreakScheduler.swift`
- Create: `Sources/RestEyes/ScreenLocker.swift`
- Modify: `Sources/RestEyes/main.swift`
- Modify: `Tests/RestEyesCoreTests/ConfigTests.swift`、`Tests/RestEyesCoreTests/BreakSchedulerTests.swift`
- Modify: `README.md`、`Resources/Info.plist`(版本 0.1.3 / build 4)

**Interfaces:**
- Consumes: 现有 `Config`/`BreakScheduler`/AppDelegate 接线
- Produces: `Config.lockAfterRest: Bool`(默认 true,键 `lock_after_rest`,on/off);`RestEndReason`(`completed`/`unlocked`/`wake`)+ `BreakScheduler.onRestEnded: ((RestEndReason) -> Void)?`;`ScreenLocker.lock()`

**语义(实现依据,与 spec 一致):**
- `onRestEnded` 仅在 resting 结束时触发,原因三分:tick 到期 → `.completed`;`unlock(now:)` → `.unlocked`;`systemDidWake` 中 resting 已过点 → `.wake`。进入休息、其他相位迁移一概不触发。
- 回调顺序:`onPhaseChange(.working)`(startWork 内部)→ `onRestEnded` → (tick 路径)`onTick`。UI 层因此先还原 kiosk 再锁屏。
- AppDelegate 仅在 `reason == .completed && config.lockAfterRest` 时调用 `ScreenLocker.lock()`。
- 锁屏实现:dlopen `login.framework` 取 `SACLockScreenImmediate`(Ctrl+Cmd+Q 同源,无 TCC);符号缺失时降级 `CGSession -suspend`。

- [ ] **Step 1: Config 增加 lockAfterRest**

`Sources/RestEyesCore/Config.swift`:在 `public var showCountdown: Bool = true` 之后添加属性:

```swift
    public var lockAfterRest: Bool = true
```

`defaultFileContent` 的 `show_countdown` 行之后添加:

```
lock_after_rest = on   # 休息自然结束后进入系统锁屏;手动解锁不触发(on/off)
```

`parse` 的 switch 中 `"show_countdown"` case 之后添加:
...
- [ ] **Step 2: BreakScheduler 增加带原因的休息结束回调**

`Sources/RestEyesCore/BreakScheduler.swift`:在 `TickInfo` 定义之后添加:
...
`BreakScheduler` 中 `onTick` 声明之后添加:

```swift
    public var onRestEnded: ((RestEndReason) -> Void)?
```

`tick(now:)` 中把 `case .resting, .paused: startWork(now: now)` 拆分为:
...
`unlock(now:)` 中 `startWork(now: now)` 之后添加 `onRestEnded?(.unlocked)`;
`systemDidWake` 的 `case .resting` 分支 `if now >= deadline { startWork(now: now) }` 改为:
...
- [ ] **Step 3: 新建 ScreenLocker**

`Sources/RestEyes/ScreenLocker.swift`(整文件):
...
- [ ] **Step 4: main.swift 接线**

`Sources/RestEyes/main.swift` 的 `wire()` 中,`overlay.onUnlockRequested = ...` 块之前添加:
...
`Tests/RestEyesCoreTests/ConfigTests.swift`:`testDefaults` 中 `XCTAssertTrue(c.showCountdown)` 之后添加 `XCTAssertTrue(c.lockAfterRest)`;类内追加:
...
`Tests/RestEyesCoreTests/BreakSchedulerTests.swift` 类内追加:
...
README.md:功能列表添加一条 `- 休息结束后自动锁屏(同 Ctrl+Cmd+Q 效果,可配置关闭;手动解锁不触发)`;配置表添加一行:
...
`Resources/Info.plist`:`CFBundleShortVersionString` → `0.1.3`,`CFBundleVersion` → `4`。

- [ ] **Step 7: 提交推送 + CI 绿 + 发布 v0.1.3**

Commit message:

```
feat: optional screen lock after rest ends naturally (lock_after_rest)

BreakScheduler emits a reason-tagged rest-end callback (completed/unlocked/
wake); the app locks the screen via SACLockScreenImmediate (Ctrl+Cmd+Q
equivalent, CGSession fallback) only for natural completion with the new
lock_after_rest config key enabled (default on).

Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>
```

推送 main,CI 绿(预期 `Executed 49 tests, with 0 failures`:Config 18 + Scheduler 30 + Format 1);`git tag v0.1.3 && git push origin v0.1.3`,盯 tag run 绿,`gh release view v0.1.3` 验证附件。

## 增量:v0.1.4 功能

### Task 10:解锁直接结束休息(wake_ends_rest)

**Files:**
- Modify: `Sources/RestEyesCore/Config.swift`
- Modify: `Sources/RestEyesCore/BreakScheduler.swift`
- Modify: `Tests/RestEyesCoreTests/ConfigTests.swift`、`Tests/RestEyesCoreTests/BreakSchedulerTests.swift`
- Modify: `README.md`、`Resources/Info.plist`(版本 0.1.4 / build 5)

**Interfaces:**
- Consumes: 现有 `Config`/`BreakScheduler`;`RestEndReason.wake`(Task 9)
- Produces: `Config.wakeEndsRest: Bool`(默认 true,键 `wake_ends_rest`,on/off)

**语义(实现依据,与 spec 一致):**
- 休息中睡眠/锁屏后唤醒解锁:`wake_ends_rest = on`(默认)→ 无论休息剩余多少,直接 `startWork` 并发 `onRestEnded(.wake)`;`off` → 维持原行为(仅 `now >= deadline` 才解除)。
- `.wake` 原因不触发 `lock_after_rest` 锁屏(main.swift 只认 `.completed`,零改动)。
- working/warning 中的睡眠补偿逻辑不变;UI 层零改动。

- [ ] **Step 1: Config 增加 wakeEndsRest**

`Sources/RestEyesCore/Config.swift`:在 `public var lockAfterRest: Bool = true` 之后添加:

```swift
    public var wakeEndsRest: Bool = true
```

`defaultFileContent` 的 `lock_after_rest` 行之后添加:

```
wake_ends_rest = on    # 睡眠/锁屏后唤醒解锁时,直接结束休息回到工作(on/off)
```

`parse` 的 switch 中 `"lock_after_rest"` case 之后添加:
...
- [ ] **Step 2: BreakScheduler 分支加配置判断**

`Sources/RestEyesCore/BreakScheduler.swift` 的 `systemDidWake` 中,把:
...
`Tests/RestEyesCoreTests/ConfigTests.swift`:`testDefaults` 中 `XCTAssertTrue(c.lockAfterRest)` 之后添加 `XCTAssertTrue(c.wakeEndsRest)`;类内追加:
...
`Tests/RestEyesCoreTests/BreakSchedulerTests.swift`:两个既有测试改为显式测 off 行为——

把 `testWakeDuringRestBeforeDeadlineKeepsResting` 整个函数替换为:
...
把 `testRestEndReasonNotFiredWhenWakeKeepsResting` 整个函数替换为:
...
README.md:功能列表 `- 休息结束后自动锁屏...` 之后添加一条:

```
- 休息中主动锁屏/合盖,解锁后直接回到工作(可配置关闭)
```

配置表 `lock_after_rest` 行之后添加一行:
...
`Resources/Info.plist`:`CFBundleShortVersionString` → `0.1.4`,`CFBundleVersion` → `5`。

- [ ] **Step 5: 提交推送 + CI 绿 + 发布 v0.1.4**

Commit message:

```
feat: wake_ends_rest - unlocking ends an in-progress rest (default on)

Manually locking or sleeping during a rest and unlocking now returns
straight to a fresh work period instead of resuming the overlay countdown.
Fires RestEndReason.wake, so it never triggers lock_after_rest. Set
wake_ends_rest = off to restore the wall-clock resume behavior.

Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>
```

推送 main,CI 绿(预期 `Executed 51 tests, with 0 failures`:Config 19 + Scheduler 31 + Format 1);`git tag v0.1.4 && git push origin v0.1.4`,盯 tag run 绿,`gh release view v0.1.4` 验证附件。

## 增量:v0.1.5 功能

### Task 11:开机自启动(launch_at_login)

**Files:**
- Modify: `Sources/RestEyesCore/Config.swift`
- Create: `Sources/RestEyes/LoginItem.swift`
- Modify: `Sources/RestEyes/main.swift`(三个配置加载点)
- Modify: `Tests/RestEyesCoreTests/ConfigTests.swift`
- Modify: `README.md`、`Resources/Info.plist`(版本 0.1.5 / build 6)

**Interfaces:**
- Consumes: 现有 `Config` / AppDelegate 配置加载点
- Produces: `Config.launchAtLogin: Bool`(默认 true,键 `launch_at_login`,on/off);`LoginItem.sync(enabled:)`

**语义(实现依据,与 spec 一致):**
- `SMAppService.mainApp` 注册/注销;启动 + 每次配置加载时对账(on 且未注册 → register;off 且已注册 → unregister)。
- 用户在系统设置手动关闭后不强行翻转(status 非 `.enabled` 时 register 由系统决定是否生效,失败静默)。
- 无 bundle id(裸二进制)静默跳过。核心库零改动。

- [ ] **Step 1: Config 增加 launchAtLogin**

`Sources/RestEyesCore/Config.swift`:在 `public var wakeEndsRest: Bool = true` 之后添加:

```swift
    public var launchAtLogin: Bool = true
```

`defaultFileContent` 的 `wake_ends_rest` 行之后添加:

```
launch_at_login = on   # 开机自动启动(on/off)
```

`parse` 的 switch 中 `"wake_ends_rest"` case 之后添加:
...
`Sources/RestEyes/LoginItem.swift`(整文件):
...
- [ ] **Step 3: main.swift 三个配置加载点接入**

(a) `applicationDidFinishLaunching` 中,把:
...
`Tests/RestEyesCoreTests/ConfigTests.swift`:`testDefaults` 中 `XCTAssertTrue(c.wakeEndsRest)` 之后添加 `XCTAssertTrue(c.launchAtLogin)`;类内追加:
...
README.md:功能列表 `- 休息中主动锁屏/合盖...` 之后添加一条:
...
`Resources/Info.plist`:`CFBundleShortVersionString` → `0.1.5`,`CFBundleVersion` → `6`。

- [ ] **Step 6: 提交推送 + CI 绿 + 发布 v0.1.5**

Commit message:

```
feat: launch at login (launch_at_login, default on)

Register/unregister via SMAppService.mainApp, reconciled with the config
at startup and on every config reload. Manual disable in System Settings
is respected. Bare binaries without a bundle id skip silently.

Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>
```

推送 main,CI 绿(预期 `Executed 52 tests, with 0 failures`:Config 20 + Scheduler 31 + Format 1);`git tag v0.1.5 && git push origin v0.1.5`,盯 tag run 绿,`gh release view v0.1.5` 验证附件。
